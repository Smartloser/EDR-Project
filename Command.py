...
list - List all running scanners""")
            ...
            print("Unknown command. Please type 'help' for list of available commands.", end='\n>')

    def add_edr(self, command_data):
        addr = command_data[0]
        print(addr)
        try:
            ipaddress.ip_address(str(addr).strip())
            if addr.find(':') > -1:
    ...
    def list_edr(self):
        if not self.edr_manager.get_edr_list():
            print("No EDR has been initiated.")
            return    
